 ...
 *
 * It detects the format automatically from the code length after removing non-digit characters (separators).
 * (except ISBN-A, which is detected by the DOI prefix "10.")
 *
 * Once the format is detected, code integrity will be validated using check digit (unless disabled)
...
class Parser(private val rangesProvider: Provider) {
    private enum class ErrorType(val number: Int, val message: String) {
         ...
         UNEXPECTED_CHARACTERS(2, "Unexpected characters in the code"),
                  WRONG_LENGTH(3, "Code length is not matching any known format"),
    ...
    fun parse(input: String, checkIntegrity: Boolean = true): BookNumber {
        ...
        return Pair(input, BookNumber())
            .let(::extractCheckDigitAndFormat)
    ...
    private fun extractCheckDigitAndFormat(input: Pair<String,BookNumber>): Pair<String,BookNumber> {
        ...
        if (code.length in 12..13) {
            val format = when {
                hadDoiPrefix -> BookNumber.Format.ISBN_A
                hadSeparators -> BookNumber.Format.ISBN_13
                else -> BookNumber.Format.EAN_13
            }
            ...
            if (checkDigit != null && !checkDigit.isDigit()) {
                throw ParseException(ErrorType.UNEXPECTED_CHARACTERS, "$code>$checkDigit< (${format.printedName})")
            }
            val separator = when(format) {
                BookNumber.Format.ISBN_13 -> possibleSeparator
                else -> null
            }
            return Pair(
                code,
                bn.copy(metadata = bn.metadata.copy(sanitisedCode = code, format = format, checkDigit = checkDigit, separator = separator))
            )
        }

        if (code.length in 9..10) {
            val format = when {
                hadSeparators -> BookNumber.Format.ISBN_10
                else -> BookNumber.Format.EAN_10
            }
            val checkDigit = when(code.length) {
            ...
            val separator = when(format) {
                BookNumber.Format.ISBN_10 -> possibleSeparator
                else -> null
            }
            return Pair(
                code,
        ...
        if (code.length == 14) {
            val format = BookNumber.Format.GTIN_14
            if (hadSeparators) {
                throw ParseException(ErrorType.UNEXPECTED_CHARACTERS, ">$inputCode< (${format.printedName})")
            }

            val packagingIndicator = code.first().digitToIntOrNull()
                .let { if (it != null && it <= 8) it else null }
                ?: throw ParseException(ErrorType.UNEXPECTED_CHARACTERS, ">${code.first()}<${code.drop(1)} (${format.printedName})")
            val checkDigit = code.last()

            return Pair(
                code.drop(1).dropLast(1),
                bn.copy(metadata = bn.metadata.copy(sanitisedCode = code, format = format, checkDigit = checkDigit, packagingIndicator = packagingIndicator))
            )
        }
    ...
    private fun extractRegGroup(input: Pair<String,BookNumber>): Pair<String,BookNumber> {
        ...
        val group = code.take(range.length).toUIntOrNull()
        ...
        return Pair(
            ...
            bn.copy(group = group, metadata = bn.metadata.copy(groupLength = range.length))
        )
    }
    ...
    private fun extractRegistrant(input: Pair<String,BookNumber>): Pair<String,BookNumber> {
        ...
        val ranges = rangesProvider.getGroupRanges(bn.gs1, bn.group)
